        ...
        if (command=="BUTTON PRESS"){
            ...
            **************************************************/
             debug("received: "+str+"avname is: "+avname+"\n"+llList2CSV(facilitators));
              if (button=="joingame"){
                  llSay(0,"A new player ("+llKey2Name(avuuid)+") is trying to enter the game");
                      authenticatedUser= "&sloodleuuid="+(string)avuuid+"&sloodleavname="+llEscapeURL(llKey2Name(avuuid));                   
                      llMessageLinked(LINK_SET, PLUGIN_CHANNEL, "awards->joingame"+authenticatedUser+"&sloodlemoduleid="+(string)currentAwardId+"&gameid="+(string)gameid+"&avuuid="+(string)avuuid+"&avname="+llEscapeURL(llKey2Name(avuuid)), llGetScriptName()); 
                }//endif button=joingame
                else
...
// SLOODLE LSL Script Subversion Location: mod/awards-1.0/lsl/root_prim_board/btn_handler.lsl 